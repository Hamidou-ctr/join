...
/**
 * Checks if the user is logged in by sending a request to the status endpoint.
 * If the user is logged in, redirects to the main page. Logs any errors that occur during the process.
 * @async
 * @returns {Promise<void>} - Resolves when the check is complete, or logs an error.
 */
async function checkIfLoggedIn() {
    try {
        ...
        if (!response.ok) {
            console.error('Error when checking the login status: Invalid status code:', response.status);
            return;
        }
        ...
        console.error('Error when checking the login status:', error);
    }
}
