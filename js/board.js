...
/**
 * Adds a subtask to the current task being edited and saves it to the backend.
 * @param {string} subtask - The subtask to be added.
 */
let editingSubtasks = [];
async function addSubtaskToTask(subtask) {
    editingSubtasks.push(subtask);

    let taskId = document.getElementById('popupTaskId').value;
    try {
        let apiUrl = await importConfig();
        let response = await fetchToBackend(`${apiUrl}/api/add-subtask/${taskId}`, 'POST', JSON.stringify({ subtask }));
        if (!response.ok) {
            throw new Error('Failed to add subtask to the backend');
        }
        updateSubtaskListAfterEditing();
...
/**
 * Updates the progress bar for a task based on the number of completed subtasks.
 * @param {number|string} taskId - The ID of the task for which to update the progress bar.
 */
function updateProgressBar(taskId) {
    let task = tasksGlobal[taskId];
    let progressElement = document.getElementById(`${taskId}-progress`);
    let pElement = document.getElementById(`${taskId}-p`);
    ...
    if (!progressElement) {
        ...
        progressElement.id = `${taskId}-progress`;
        document.querySelector('.some-container').appendChild(progressElement);
    }
    if (!pElement) {
        pElement = document.createElement('p');
        pElement.id = `${taskId}-p`;
        document.querySelector('.some-container').appendChild(pElement);
    }
    ...
    pElement.textContent = `${totalSubtasks} Subtask${totalSubtasks !== 1 ? 's' : ''}`;
}
 ...
 * @param {string} taskId - The ID of the task whose subtasks are being updated.
 */
function updateSubtaskListAfterAddNew(taskId) {
    let subtasksElement = document.getElementById('subtasksContainer');
    subtasksElement.innerHTML = '';
    editingSubtasks.forEach((subtask, index) => {
        let isChecked = tasksGlobal[taskId]?.subTasksChecked[index] || false;
        ...
        subtaskElement.innerHTML = `
            <div id="subtask-${index}" class="subtask-item">
                ...
                <div class="subtask-actions">
                    <div class="edit-subtask" onclick="editSubtask(${index})"><img src="./assets/img/edit.svg" alt="Edit"></div>
                    <button class="remove-subtask" onclick="removeEditingSubtask(${index})"><img src="./assets/img/delete.png" alt="Delete"></button>
                </div>
            </div>
        `;
        subtasksElement.appendChild(subtaskElement);
...
/**
 * Updates the subtask list in the DOM after editing a task.
 * @param {number|string} taskId - The ID of the task whose subtasks are being updated.
 */
function updateSubtaskListAfterEditing(taskId) {
    let subtasksElement = document.getElementById('subtasksContainer');
    subtasksElement.innerHTML = '';
    editingSubtasks.forEach((subtask, index) => {
        let isChecked = tasksGlobal[taskId]?.subTasksChecked[index] || false;
        let subtaskElement = document.createElement('div');
        subtaskElement.className = 'subtask-item';
        subtaskElement.innerHTML = subtaskElementHml(index, isChecked, subtask);
        subtasksElement.appendChild(subtaskElement);
    });
}

/**
 * Removes a subtask from the list of subtasks being edited and updates the backend.
 * @param {number} index - The index of the subtask to remove.
 */
async function removeEditingSubtask(index) {
    let taskId = document.getElementById('popupTaskId').value;
    try {
        let apiUrl = await importConfig();
        let response = await fetchToBackend(`${apiUrl}/api/remove-subtask/${taskId}/${index}`, 'PATCH');
        if (!response.ok) {
            throw new Error('Failed to remove subtask from the backend');
        }
        editingSubtasks.splice(index, 1);
        document.getElementById('subtask-' + index).parentElement.remove();
    } catch (error) {
...
async function displayTasks(tasks) {
    const categories = ['todo', 'in-progress', 'awaiting-feedback', 'done'];
    for (const category of categories) {
        let box = document.getElementById(`${category}-box`);
        box.innerHTML = '';
        let hasTasks = false;
        for (const firebaseTaskId of Object.keys(tasks)) {
            let task = tasks[firebaseTaskId];
            if (task.state === category) {
                hasTasks = true;
        ...
        if (!hasTasks) {
            ...
            noTasksDiv.textContent = getNoTaskMessage(category);
            noTasksDiv.className = 'no-tasks';
            box.appendChild(noTasksDiv);
        }
    }
    hideLoader();
}
...
async function createTaskElement(task, firebaseTaskId) {
    ...
    if (task.assigneeNames) {
        let assigneesCount = task.assigneeNames.length;
        let maxVisibleAssignees = 4;
        let assigneesToShow = task.assigneeNames.slice(0, maxVisibleAssignees);
        for (let fullName of assigneesToShow) {
        ...
        assigneesCount > maxVisibleAssignees ? assigneesHTML += `<div class="assignedto" style="display: inline-block;color: black;background: none;font-weight: bold;margin-left: 4px;;">+${assigneesCount - maxVisibleAssignees}</div>` : null;
    }
...
function closePopup() {
    ...
    !document.getElementById('popupContainer').classList.contains("d-none") ? document.getElementById('popupContainer').classList.add('d-none') : null;
    toAssignUser = [];
    resetInputFields();
    hideInputLabels()
}
...
/**
 * Sets up event handlers for all checkboxes, ensuring they trigger the appropriate user toggle logic without propagating the event.
 */
