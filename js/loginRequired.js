...
function toggleLogout() {
    const logoutMenu = document.getElementById('header-logout');
    const isVisible = logoutMenu.style.display === 'flex';
...
/**
 * Hides the logout button in the UI.
 * @returns {void}
 */
function hideLogout() {
...
/**
 * Closes the logout menu if clicked outside of it.
 */
window.onclick = function(event) {
    const logoutMenu = document.getElementById('header-logout');
    if (event.target !== logoutMenu && !logoutMenu.contains(event.target) && event.target.id !== 'profile-actions') {
        hideLogout();
    }
};
...
/**
 * Loads the sidebar content, including logo, navigation links, and legal links, into the sidebar header element.
 * Dispatches a custom event 'sidebarLoaded' after the sidebar is populated.
 */
...
/**
 * Toggles the visibility of the success popup by adding or removing the 'd-none' class.
 */
...
/**
 * Sets a success message in the success popup, displays it, and automatically hides it after 8 seconds.
